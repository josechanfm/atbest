...
<div>
    <a-menu v-model:openKeys="openKeys" v-model:selectedKeys="selectedKeys" mode="inline" theme="light" :inline-collapsed="collapsed">
        ...
        <template v-if="$page.props.auth?.user?.is_organizer">
            <!-- Organizer 管理員 -->
            <a-menu-item v-for="organizer in $page.props.auth.user.organizer" :key="organizer.id">
                <template #icon>
                    <SettingOutlined />
                </template>
                <span>
                    <inertia-link :href="route('organizer.organizations.show', organizer.id )">
                        {{ organizer.abbr }} {{ $t("setting") }}
                    </inertia-link>
                </span>
            </a-menu-item>
        </template>
